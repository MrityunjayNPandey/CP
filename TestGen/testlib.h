...
void InStream::reset() {
    ...
    if (!stdfile)
        if (NULL == (file = std::fopen(name.c_str(), "rb"))) {
            if (mode == _output) quits(_pe, std::string("File not found: \"") + name + "\"");
        }
...
void InStream::readWordTo(std::string& result) {
    ...
    if (cur == EOF) quit(_unexpected_eof, "Unexpected end of file - token expected");
...
long long InStream::readLong(long long minv, long long maxv, const std::string& variableName) {
    ...
    if (result < minv || result > maxv) {
        if (variableName.empty())
            quit(_wa,
                 ("Integer " + vtos(result) + " violates the range [" + vtos(minv) + ", " + vtos(maxv) + "]").c_str());
        else
...
double InStream::readReal(double minv, double maxv, const std::string& variableName) {
    ...
    if (result < minv || result > maxv) {
        if (variableName.empty())
            quit(_wa,
                 ("Double " + vtos(result) + " violates the range [" + vtos(minv) + ", " + vtos(maxv) + "]").c_str());
        else
...
void InStream::readStringTo(std::string& result, const pattern& p, const std::string& variableName) {
    ...
    if (!p.matches(result)) {
        if (variableName.empty())
            quit(_wa,
                 ("Line \"" + __testlib_part(result) + "\" doesn't correspond to pattern \"" + p.src() + "\"").c_str());
        else
